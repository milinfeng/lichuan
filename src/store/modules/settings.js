...
const mutations = {
  toggleCollapse(state) {
    ...
    const leftDrawer = document.querySelector('.left-drawer .ant-drawer-content-wrapper')
    console.log(leftDrawer);
    if (leftDrawer) {
      if (state.collapse) {
        // 折叠起来了 
        leftDrawer.classList.remove('drawer-translate-to-right')
        leftDrawer.classList.add('drawer-translate-to-left')
        // leftDrawer.style.left = '80px'
      } else {
        // 展开了
        leftDrawer.classList.remove('drawer-translate-to-left')
        leftDrawer.classList.add('drawer-translate-to-right')
      }
    }
